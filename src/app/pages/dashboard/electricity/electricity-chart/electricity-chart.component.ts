...
export class ElectricityChartComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.themeSubscription = this.theme.getJsTheme().delay(1).subscribe(config => {
      ...
      this._webWorkerService.run(this.createData).then((data) => {
        ...
        this.createOptions(eTheme, categoryData, lineData);
  ...
  createData() {
    ...
    const categoryData = data.map(i => i.label);
    const lineData = data.map(i => i.value);
  ...
  createOptions(eTheme: any, categoryData, lineData) {
    this.option = {
      grid: {
        left: 0,
        top: 0,
